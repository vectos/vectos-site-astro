...
title: "Building the edge-peripheral a ESP32 edge device"
...
To accomplish this we switched from an archictecture where the hardware peripheral connects via WiFi to the cloud backend and needs to authenticate itself with [Auth0](https://auth0.com/) to a Bluetooth Low Energy (BLE). To connect and authenticate with WiFi can be resource intensive and the connection to any HTTP + TLS service as well (as you need encryption). Other companies like Philips Hue and Tado are also using with a hub (central) architecture which is more power efficient.
...
Like I've mentioned in the start of the series, my brother Hans de Joing did the design of the PCB. We've used the following components which communicate are all connected to the microcontroller ESP32. There are different protocols you can use in embedded systems, but we only need to use I2C.
...
Do you have an idea to build a new device which interacts with cloud technology which measures on controls things in the outside world? That's internet of things, I might be able to help out! Read more on this
