...
## Why continous integration and delivery?
...
(as refered in the The Phoenix Project). If you can build, test and release your
software in automated way it's easy to verify if it works correctly and have it
up on a preview/staging environments for product owners/QA/ops (the silos) and
other teams to verify if the new artifacts works.
...
- **Simple enough** - Build systems can become projects and studies on their
  own. For examples _Bazel_, which is an exceptional cool technology but
...
- **Bazel**: While powerful and fast for large projects, Bazel is complex to set up and maintain. It requires a dedicated engineering team to manage its configuration and integration, which is not feasible for our timeline and resources.
...
<Aside variant="caution">Another import detail is here including `this.source.directory("backend")` with a `filter` applied. This filter only includes files which are suspectible for changes and _will_ impact the caching mechansim. If none of these files changed, it won't trigger a rebuild of that layer.</Aside>
...
To see how the build is performing you can open a tracing web terminal like seen here. This can also be used in CI/CD enviroments

![Dagger trace](/img/blog/mycelium/dagger-trace.png)

This can be used to view what parts of the build are slow, in this case the build used cached was finished in a few seconds!

## Conclusion

Dagger is a very promising piece of technology for building programmable, composable pipelines. It offers powerful features like caching, tracing, and portability, while letting you define pipelines in real programming languages instead of YAML. That said, the surrounding ecosystem — the "Daggerverse" — is still young. Setting up caching for hobby projects can require either a delicate self-hosted Kubernetes setup or a paid service like [Depot](https://depot.dev). For enterprise adoption, it might be slightly too early given the maturity of the tooling and ecosystem. Still, Dagger shows strong potential and is worth keeping a close eye on as it continues to evolve.
